		           ...
		           <div class="border p-3 rounded">
		            <form class="row g-3"  id="myform" method="post"   >
                        <div class="col-12">
                          ...
                          <input type="text" name="cityname" class="form-control" id="cityname" placeholder="Enter City Name" required>
                          <input type="hidden" name="event" value="updateCity">
                          <input type="hidden" id="cityid" name="cityid" value="">
                        </div>
                       
                        <div class="col-12">
                          <input type="submit" class="btn btn-primary px-4"  id="submit" value="Update City">
                        </div>
                      </form>
	    			...
	    			success:function(data,textStatus,jqXHR){ 					 
	    				...
	    				for (var key in data) {
	    					if (data.hasOwnProperty(key)) {
	    					  s+="<tr>";
	    					  s+="<td>"+data[key].city_id+"</td>";	    					 
	    					  s+="<td>"+data[key].city_name+"</td>";
	    					  s+="<td><div class='table-actions d-flex align-items-center gap-3 fs-6'>";
	    					  s+="<a  class='text-info city_edit' data-bs-toggle='tooltip' data-bs-placement='bottom' title='Edit' data-id='"+data[key].city_id+"' id='"+data[key].city_id+"'><i class='bi bi-pencil-fill'></i></a>";
	    					  s+="<a  class='text-danger city_delete' data-bs-toggle='tooltip' data-bs-placement='bottom' title='Delete' data-id='"+data[key].city_id+"' id='"+data[key].city_id+"'><i class='bi bi-trash-fill'></i></a>";
	    					  s+="</div></td></tr>";
	    					}
	    				}
	    				$('#view_city').html(s);
	    		...
	    		error:function(jqXHR,textStatus,errorThrown){
				 ...
				 $(document).on('click', '.city_delete', function() {
	              ...
	              $.ajax({
					    type:'POST',
					    url:'../cityServlet',
						data:{'cityId':cityId,'event':'city_delete'},
						method:'POST',
						dataType:'JSON',
						success:function(data,textStatus,jqXHR){
	  						if(data.trim() ==='done'){	  							
		...
		<script>
			 $(document).on('click', '.city_edit', function() {
				...
				$.ajax({
				    type:'POST',
				    url:'../cityServlet',
					data:{'cityId':cityId,'event':'fatchCityById'},
					method:'POST',
					dataType:'JSON',
					success:function(data,textStatus,jqXHR){
						//console.log(data);
		  	...
		  	$(document).ready(function(){
		  		...
		  		$("#myform").on('submit',function(event){
					...
					$.ajax({
		  				...
		  				success:function(data,textStatus,jqXHR){
			  				if(data.trim() ==='done'){			  					
			  					window.location.href = 'viewCity.jsp';		  				
			  		  		}else{
					  		  		$.toast({
					  		  	    text: "Something went wrong on server!", 
					  		  	    heading: 'Failed...', 
