	    			...
	    			success:function(data,textStatus,jqXHR){ 					 
	    				...
	    				for (var key in data) {
	    					if (data.hasOwnProperty(key)) {
	    					  ...
	    					  s+="<img src='assets/adminImg/"+data[key].image+"'class='rounded-circle' width='44' height='44' alt=''>";
	    					  ...
	    					  s+="<a  class='text-info admin_edit' data-bs-toggle='tooltip' data-bs-placement='bottom' title='Edit' data-id='"+data[key].id+"' id='"+data[key].id+"'><i class='bi bi-pencil-fill'></i></a>";
	    					  s+="<a  class='text-danger admin_delete' data-bs-toggle='tooltip' data-bs-placement='bottom' title='Delete' data-id='"+data[key].id+"' id='"+data[key].id+"'><i class='bi bi-trash-fill'></i></a>";
	    					  s+="</div></td></tr>";
	    					}
	    				}
	    				$('#view_admin').html(s);
	    		...
	    		error:function(jqXHR,textStatus,errorThrown){
				 ...
				 $(document).on('click', '.admin_delete', function() {
	              ...
	              $.ajax({
					    type:'POST',
					    url:'../adminServlet',
						data:{'adminId':adminId,'event':'admin_delete'},
						method:'POST',
						dataType:'JSON',
						success:function(data,textStatus,jqXHR){
	  						if(data.trim() ==='done'){	  							
		...
		<script>
			 $(document).on('click', '.admin_edit', function() {
				...
				$.ajax({
				    type:'POST',
				    url:'../adminServlet',
					data:{'adminId':id,'event':'fatchAdminById'},
					method:'POST',
					dataType:'JSON',
					success:function(data,textStatus,jqXHR){
						$("#id").val(data[0].id);
